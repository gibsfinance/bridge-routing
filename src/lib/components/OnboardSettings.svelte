<script lang="ts">
  ...
  import { activeOnboardStep, showGuide } from '../stores/storage.svelte'
  const toggleHelp = () => {
...
<header class="flex flex-col items-center justify-between gap-2">
  <div class="flex flex-row w-full items-center justify-between relative">
    <div class="flex flex-row grow items-center gap-1 h-12">
      <Button
        onclick={toggleHelp}
        class="flex flex-row items-center italic gap-1 pr-1 border-2 rounded-full {showGuide.value
          ? ' '
          : ' border-transparent'}">
        <Icon icon="material-symbols:help" mode="svg" class="size-9" />
        <span class="text-sm pr-1 leading-6" class:invisible={!showGuide.value}>Dismiss</span>
      </Button>
    </div>
